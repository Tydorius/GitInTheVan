...
REM Check Program Files (system-wide)
...
REM Check nvm-windows installations (latest version)
...
REM Check fnm (Fast Node Manager) installations
...
REM Node not found anywhere
echo DEBUG: Node.js not found in any location.

REM Offer to install via winget
where winget >nul 2>&1
if errorlevel 1 (
...
REM Re-check PATH after install (may not take effect until new shell)
where node >nul 2>&1
if not errorlevel 1 (
    for /f "delims=" %%n in ('where node') do set "NODE_CMD=%%n"
    where npm >nul 2>&1
...
REM Check standard locations after install
if exist "C:\Program Files\nodejs\node.exe" (
    set "NODE_CMD=C:\Program Files\nodejs\node.exe"
    set "NPM_CMD=C:\Program Files\nodejs\npm.cmd"
    goto :node_found
...
REM Verify Node version is 18+ (24+ recommended, 18+ required for Vite 8)
